...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center font-body font-semibold rounded-md transition-all active:scale-[0.98] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 ${
        noWrap ? 'whitespace-nowrap' : 'text-center leading-tight'
    ...
    >
      {isLoading ? (
        ...
        <>
          ...
          {children && (
            <span className={noWrap ? 'truncate' : 'text-center leading-tight'}>
              {children}
            </span>
          )}
